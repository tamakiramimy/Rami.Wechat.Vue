  ...
  <el-row>
    <el-row style="margin-top:10px;padding:10px;background-color:white;">
      <el-form :inline="true">
        <el-form-item label="名称">
          <el-input placeholder="名称" v-model="para.Entity.AutoName"></el-input>
        </el-form-item>
        <el-form-item label="关键字">
          <el-input placeholder="关键字" v-model="para.Entity.KeyWords"></el-input>
        </el-form-item>
    ...
    <el-row>
      <el-table border style="width: 100%" :data="lstDatas">
        <el-table-column label="名称" prop="AutoName"></el-table-column>
    ...
    <el-dialog title="编辑" width="30%" :visible.sync="isShowEdit">
      <el-form ref="form" label-width="20%">
        <el-form-item label="微信昵称">
          <el-input v-model="entity.PrevNick"></el-input>
        </el-form-item>

        <el-form-item label="OpenID">
          <el-input v-model="entity.OpenID"></el-input>
        </el-form-item>
      </el-form>

      <el-row style="text-align:center;">
        <el-button size="small" round type="primary" :disabled="isDisSave" @click="actPrev">提交</el-button>
...
export default {
  ...
  methods: {
    ...
    getDatas: function () {
      let that = this;
      that.$http.post("WxFunc3/GetAutoKeywords", that.para).then(res => {
        if (res.IsSucc) {
          let data = res.Data;
    ...
    // 新增
    addItem: function () {
      // let url = '@Url.Action("WxAutoRespEdit", "WxFunc")?id=' + 0;
      // window.open(url, "_blank");
      this.$router.push('/baseconf/wxautorespedit?id=0');
    },
    // 修改
    editItem: function (item) {
      // let url = '@Url.Action("WxAutoRespEdit", "WxFunc")?id=' + item.Id;
      // window.open(url, "_blank");
      let url = `/baseconf/wxautorespedit?id=${item.Id}`;
    ...
    actPrev: function () {
      ...
      that.$http.post('WxFunc3/PrevKeyword', item).then(res => {
    ...
    updItem: function (item) {
      let that = this;
      that.$msg.showComfirm(that, '你确定要修改此关键字配置的状态吗?', () => {
        that.$http.post('WxFunc3/UpdAutoKeyword', item).then(res => {
          that.$msg.showMsg(that, res.Message, res.IsSucc ? 0 : 1);
          if (res.IsSucc) {
            that.getDatas();
          }
